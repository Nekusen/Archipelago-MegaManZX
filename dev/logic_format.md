...
Four modules consume the document. `logic/document.py` is the shared core: atoms, requirement parsing, region membership
of every node, validation and the text export. It imports nothing from Archipelago, so the editor and the command-line
tools load it on their own. `logic/rules.py` holds what is not drawn: the door table, the key rule, the Transerver rule, the
area-label rule and the starting room. `regions.py` turns the document into Archipelago regions, entrances, locations
and events, and `logic/bosses.py` adds the per-boss requirements from the player's YAML. Behind all of this is `data.py`, a
...
of one Hub floor. When both doors of a floor lead into the same room (floor A: the two halves of A-2, with the
starting point between them), the corridor is a same-room edge named with its landing position.
...
so `FX` alone is enough as well. A tier that is absent contributes nothing. The world ships at `normal` (`TIER` in
`logic/rules.py`); expert is drawn in the document but not offered as an option yet.
...
`main` region of the starting room (the `room` of the `starting_transerver` record in `data.STARTING_TRANSERVERS`),
with that room's entry requirement. `validate` treats the `main` region of every starting room as an entry.
...
lives in `client/bossrush.py` and plays no part in generation.

## Validation

The validator in `logic/document.py` returns errors, warnings, the number of `unsure` rules and the number of unplaced
