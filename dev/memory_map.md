...
| 0x021045D3.6 | bit | MODEL_PART2 | second half of PX (flag 62, the "visited" slot of the non-existent subarea 12) | both |
| 0x021045DB.0 | bit | HUGATE_FLAG_INDEX | Model Hu owned (flag 120, the "visited" slot of the hub itself, -1 in the map table); only tested with the Hu gate patch. Flag 136 (0x021045DD.0), used before, is the I-2/I-5 link of the world map | both |
...
| 0x021045FB.4 | bit | PEOPLE_FREED | cell of I-3 broken; the Save The People handler waits for it | client |
| 0x021045FE.0 | bit | PEOPLE_HURRICAUNE | Hurricaune beaten, set by the I-3 room script | client |
...
| 0x02104605.6, 07.0, 0D.3, 10.0 | bits | MODEL_POSSESSION | first half of HX, FX, LX, PX (flags 462, 472, 523, 544: one past the end of each series of collected disks, B/M/E/O) | both |
| 0x02104612.1, 13.3, 19.6 | bits | MODEL_PART2 | second half of HX, FX, LX (flags 561, 571, 622: one past the end of the B/M/E series of disks read in the database; 545-642 are those "read" bits) | both |
| 0x0210461D.5-7, 1E.0-4 | bits | ITEMS grant | ITEM B chips owned (1.6) | client |
| 0x02104623.0-0x02104627.3 | bits | (not free) | first talk with each townsperson (flags 696-731, table 0x020EB248 indexed by role); 731 is the Oeillet guardian of C-1. The apworld used 720-731 for the biometal halves until 2026-09-15 | game |
...
|---|---|---|---|---|
| Hu | 0 | 0x021045DB.0 with the Hu gate; always owned in vanilla | | |
| X | 1 | 0x021045CF.7 | | |
| ZX | 2 | 0x021045D0.0 | | |
| HX | 3 | 0x02104605.6 | 0x021045D0.1 Hivolt, 0x021045D1.1 Hurricaune | 0x02104612.1 |
| FX | 4 | 0x02104607.0 | D0.5 Fistleo, D1.5 Flammole | 0x02104613.3 |
| LX | 5 | 0x0210460D.3 | D0.3 Lurerre, D1.3 Leganchor | 0x02104619.6 |
| PX | 6 | 0x02104610.0 | D0.7 Purprill, D1.7 Protectos | 0x021045D3.6 |
| OX | 7 | 0x021045D2.1 | | |
...
| 3 | Pass The Test | DF.1 | 0x99 | E0.0 | 9, 10 |
...
Pass The Test is detected by E0.0, which the Report sets (branch 0x99 of
0x02031028). E0.3 is a story milestone, not the completion: the hub room
script (overlay 115, 0x02194408, state 1) sets it with the Operator's call
only once both Locate Giro (DE.7) and Pass The Test (E0.0) are reported, so
reporting Pass The Test first left the location unsent (and the mission
accepted again on every visit to C). With E0.3 set, walking into the X floor
of the hub (state 0, x 256-511, y 5504-5759) sets E0.4, takes back the four
Computer Chips and the Stuffed Animal (0x0210461E.7, 0x0210461F.0-3) and
starts the Troop chain below.
...
on; the client ORs in the bits of the received access items. The Reports of
Locate Giro and Pass The Test also set X-1 in vanilla; the ROM patch removes
that write (rom_patches.md, Area X access). Transport is
...
94 disks are layout entities (kind 6, subkind 1, role = series, modifier =
number). Disk E-47, flag 519 (0x0210460C.7), is not: the balloon of H-1
(kind 5, subkind 0x44, (3656, 682), coords index 29) creates it when it spawns
with FUN_020a4140(2, 47, balloon, (0, 36 px)). The spawner returns 0 when
FUN_0200932c finds the bit set, and the balloon then idles in state 3.
Otherwise it creates a kind 3 entity: +0x14 series, +0x15 number, +0x30
carrier, +0x64/+0x68 offset from it. The think follows the balloon and, when
touched, calls the setter and the label routine 0x020A3FC0. The balloon bobs
in sub-state 1 and flies off in sub-state 2 once the disk's state reaches 2.
Its class is in the H-1 room overlay (init 0x02197768, state table
0x02198B60, entry 68 of 0x020E9450), and it is the spawner's only caller.
...
the step is 6, the subarea is not 0, HP > 0 and the state is 0x500.

The step becomes 6 on the frame New Game or Continue is requested and does
not drop below 6 until the next Game Over or return to the title. The game
initialises the load buffer at 0x021602A8 with its new-game template by DMA
during the title load at step 3, Continue restores the save into it in the
0x..03 states of the data select, and nothing else writes it while the step
is 3 or 5; the skip cave overwrites it with the golden image on New Game,
so what it held before does not matter (exp798, exp799). The sequences the
client sees:
...
Transerver console pad is at (384, y - 17), the boss-room door is reached at
x <= 368. Floor C is the exception: its Transerver console is at (896, y - 17),
in a chamber of its own whose only door leads to C-2 (5056,1088), below the
fountain; the x = 384 chamber of that floor holds a DATA console without
Transport, with doors to C-2 (4016,928) and D-1. `data.HUB_PAD_X` (generated
from the hub's console entities, kind 5 subkind 28 role 0) gives the x per
floor; the client fell back to x = 384 on every floor until 2026-09-16.
...
UP does nothing. Teleports therefore target (HUB_PAD_X, y - 17).
...
| 0x020A4140 | spawn a carried disk (series, number, carrier, offset); only caller the H-1 balloon at 0x021977AC (overlay 81) | Disk E-47 (1.7) |
| 0x020A40C0, 0x020A3FF4, 0x020A3FC0 | carried disk (kind 3) init, think with pickup, label | 0x020A40C6, 0x020A40CE and 0x020A3FF8 call the carried icon cave; 0x020A4058 the AP gate |
...
| 0x020EB8B0 | handler tables per pickup kind (0 refill, 1 disk, 2 Life Up / Sub Tank, 3 carried disk) | the disk stub sets state 4 (release) |
...
| 0x02093B4C, 0x02093BE4; 0x020933A4, 0x02093462 | Operator console dialogue, Transerver variant (state 2) and plain computer variant (state 8; initial state per variant byte in the table at 0x020EA11C); the beq in each that skips the Yellow Card Key script (0x02094288) | both beq become b: the key dialogue never runs |
...
| 0x020C82E8 | 144 | ICON_CARRIED_CAVE | attach (+0), anim (0x020C8308), retry (0x020C832C) of the carried disk, looked up through its carrier |
...
| 0x020CB460 | 48 | SKIP_CAVE | New Game to the copy cave when the carousel step is 6 |
...
| 0x020CB560 | 40 | SKIP_COPY_CAVE | copies the golden image into the load buffer, then LOAD |
...
| 0x020CB800 | 258 | PICKUP_AP_CAVE | AP gate (+0) and stubs refill +88, disk +124, Life Up +172, Sub Tank +204, carried disk +236 |
...
| 0x02191600 | 0x4F4 | GOLDEN_IMAGE_RAM | the slot's golden image, an autoload section the boot code copies here (section 5) |
...
| 0x1000 | 0x80 | AP_MAGIC_OFFSET | +0 "MZXAP\0", +8 u32 world version (`major << 16 | minor << 8 | build`, `rom.pack_version`; the client refuses another version), +0x10 slot name (63 B + NUL), +0x50 seed name (31 B) |
...
scene from it; the skip cave copies the slot's golden image into it on New
Game (redirected to LOAD), so a new game starts in the post-tutorial hub
with or without the client. The same layout is the DATA SELECT slot content.
rom/golden.py builds the baseline from named fields (`baseline_image`); the
two tables the game copies into every new save, the weapon of each model
(16 bytes at 0x020DF794, descriptor +0x24) and the default controls (20 bytes
at 0x020DF708, descriptor +0x54), are read from the player's ARM9 by
`add_rom_tables` in patch_arm9, so the world ships no byte of the game. The
slot's image goes into the .apmmzx as `golden_image.bin` without those tables
and into the ARM9 as an autoload section at 0x02191600 (GOLDEN_IMAGE_RAM,
section 4).
...
with floor A's destination bit (0x02104627.4) set, Normal difficulty with 2
lives, Model X owned, mirrors equal to the live copies. On top of it the
builder applies the slot options: the start point (`data.STARTING_TRANSERVERS`,
generated from the hub floor table: spawn in both descriptors, scene word,
and of the 13 destination bits of 1.5 only the starting floor's, so
`guardian_base` spawns at (384, 5711) with 0x02104629.0 set and floor A's
bit cleared), Model X kept or revoked, the starting model's ownership bit,
the active model, the character in both copies, and for HX/FX/LX/PX a level
4 for the first boss of the pair plus a WE bar of 16. Verified cold in
DeSmuME: the LOAD enters the hub at the image's spawn, the floor script
keeps the bit of the floor stood on and no other, the Operator menu offers
Transport only with two or more bits, and the floor X door leads to X-1
(640, 1120) and back (exp797, exp797b). With the image baked in the ROM and
nothing written from outside, New Game lands on the image's spawn after a
power cycle and after a Game Over in another room; the only writer of the
autoload section is the boot code, and it survives play and teleports
(exp799, exp799b).
...
The parser and the icon recipe live in `rom/icons.py`; the formats are these.

**fnt set, static**: a 0x14-byte header (u32 tile offset = 0x14, u16 tile length, u16 0x18,
u16 tile length / 4, u16 flags 0x8020 = 4bpp or 0x8040 = 8bpp, u32 tile length + 8, u16 palette
length, u16) + tiles + palette (BGR555). Tiles are 8x8, 1D mapped, low nibble = left pixel.

**fnt set, chained (dynamic)**: N such headers back to back, one per chunk, each relative to
itself: chunk k's tiles are at k*0x14 + its u32, its palette at k*0x14 + (the u32 at +0xC) + 12.

**dat set**: u32 8, u32 table length, u32 4, then the frame table at +0xC (per frame: u16
offset from the table start, u8 entry count, u8 chunk), entries = u16 attr | s8 dx | s8 dy with
attr = tile:10 hflip:1 vflip:1 size:2 shape:2 (one tile index = 128 bytes), then the animation
scripts at 4 + table length (u16 offset[n]; pairs (frame, ticks), 0xFE loop, 0xFF end). The game
draws a frame's entries from the last to the first.
