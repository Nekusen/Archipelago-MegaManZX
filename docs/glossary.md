...
- golden image: the save image of a fresh post-tutorial game that the client writes to the LOAD
  buffer while the title is up, patched per slot; New Game loads it and skips the tutorial.
...
- hub, hub floor: the Guardian base Transerver room (z01). It stacks one floor per area, each
  with its console; teleports land on the floor's console pad.
